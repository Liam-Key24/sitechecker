...
export default function ScoreBadge({
  ...
  if (noWebsite) {
    ...
    return (
      <span
        className={`inline-flex items-center rounded-full font-medium bg-red-100 text-red-800 ${sizeClasses[size]}`}
      >
        {score === null ? 'No Website' : `${score}/100 - No Website`}
      </span>
    );
  }
  ...
  if (score === null) {
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 ${size === 'lg' ? 'text-sm px-3 py-1' : ''}`}>
        {analyzed ? 'Analyzed (no score)' : 'Not Analyzed'}
      </span>
    );
  }
  ...
  const getColor = (score: number) => {
    ...
    return 'bg-primary/25 text-gray-900';
  };
