...
export default function ResultsPage() {
    ...
    const fetchResults = async () => {
      ...
      try {
        ...
        if (!fetchedBusinesses || fetchedBusinesses.length === 0) {
          ...
          // Auto-analyze businesses with websites (run after state is set)
          const businessesWithWebsites = fetchedBusinesses.filter((b: Business) => b.website && !b.final_score);
          if (businessesWithWebsites.length > 0) {
            console.log(`Auto-analyzing ${businessesWithWebsites.length} businesses with websites...`);
        ...
        alert(`Error: ${error instanceof Error ? error.message : 'Failed to fetch results'}`);
        setBusinesses([]);
      } finally {
        setLoading(false);
      }
  ...
  }, [location, category, keywords, limit, handleAnalyze]);
  ...
  const handleAnalyzeAll = async () => {
    ...
    const businessesWithWebsite = businesses.filter((b) => b.website && !b.final_score);
    
    for (const business of businessesWithWebsite) {
      await handleAnalyze(business.id);
  ...
  const handleExportCSV = () => {
    ...
    const headers = ['Name', 'Website', 'Address', 'Phone', 'Category', 'Score', 'Weakness Notes'];
  ...
  const getFilteredBusinesses = () => {
    return businesses.filter((b) => {
      ...
      if (filters.scoreMin && (b.final_score === null || b.final_score < parseInt(filters.scoreMin))) return false;
      if (filters.scoreMax && (b.final_score === null || b.final_score > parseInt(filters.scoreMax))) return false;
      return true;
    });
  };
